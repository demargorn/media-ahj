      ...
      notes.forEach((note) => {
         ...
         if (revealBtn) {
            revealBtn.addEventListener('click', (ev) => {
               ev.preventDefault();
               if (
                  note.classList.contains('full-item') &&
                  noteText.classList.contains('full-content')
               ) {
...
function newNote(geoStatus) {
   ...
   const formattedTime = `${day.substr(-2)}.${month.substr(-2)}.${year} ${hours.substr(
      -2
...
function showModal() {
   ...
   enterCoordsBtn.addEventListener('click', (e) => {
      e.preventDefault();
